...
class RecipeReadSerializer(serializers.ModelSerializer):
    ...
    ingredients = IngredientRecipesReadSerializer(many=True, source='ingredientrecipes')
    ...
    def get_is_in_shopping_cart(self, obj):
        ...
        if user.is_anonymous:
            return False
        return models.ShoppingCart.objects.filter(user=user, recipe=obj).exists()


class RecipeWriteSerializer(serializers.ModelSerializer):
    author = CustomUserSerializers(read_only=True)
    ingredients = IngredientRecipesWriteSerializer(many=True)
    tags = serializers.PrimaryKeyRelatedField(queryset=models.Tags.objects.all(), many=True)
    image = Base64ImageField(required=False, allow_null=True)
    ...
    def validate(self, value):
        ...
        for ingredient in ingredients:
            if ingredients.count(ingredient) > 1:
                raise serializers.ValidationError('Выбранные игредиенты не могут повторяться')
        return value
    ...
    def create(self, validated_data):
        ...
        for ingredient in ingredients:
            ...
            ingredient = get_object_or_404(models.Ingredients, pk=ingredient['id'])
            models.IngredientRecipes.objects.create(recipe=recipe, ingredient=ingredient, amount=amount)
        return recipe
    ...
    def update(self, instance, validated_data):
        ...
        for ingredient in ingredients:
            amount = ingredient['amount']
            ingredient = get_object_or_404(models.Ingredients, pk=ingredient['id'])
            models.IngredientRecipes.objects.update_or_create(recipe=instance, ingredient=ingredient,
                                                    amount=amount)
